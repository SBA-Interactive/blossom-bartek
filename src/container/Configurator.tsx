...
export default function ConfiguratorPage() {
  ...
  const OptionButton = ({ selected, onClick, children }: { selected: boolean, onClick: () => void, children: React.ReactNode }) => (
    <Button 
      ...
      className={cn(
        "h-auto p-4 text-base md:text-lg flex flex-col items-center justify-center gap-2 transition-all duration-200",
        "border-2",
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-24">
          ...
          <div className="sticky top-24 self-start">
            <Card className="overflow-hidden shadow-lg">
              <CardContent className="p-0">
                <div className="aspect-[1/1] bg-muted flex items-center justify-center p-8 relative">
                  <img 
                  ...
                  <img 
                    ...
                    className="relative z-10 h-3/4 max-h-[400px] transition-transform duration-300 transform group-hover:scale-105"
          ...
          <div>
            ...
            <OptionGroup title="1. Choose Your Bottle">
              {BOTTLE_OPTIONS.map(bottle => (
                <OptionButton key={bottle.id} selected={selectedBottle.id === bottle.id} onClick={() => setSelectedBottle(bottle)}>
                  <img src={bottle.image} alt={bottle.name} className="h-24 w-auto" />
            ...
            <OptionGroup title="2. Select Size">
              {SIZE_OPTIONS.map(size => (
                <OptionButton key={size.id} selected={selectedSize.id === size.id} onClick={() => setSelectedSize(size)}>
                  <span className="text-2xl font-semibold">{size.name}</span>
                </OptionButton>
              ))}
            </OptionGroup>
            
            <div className="mb-8">
                <h3 className="text-2xl font-heading font-semibold mb-4">3. Compose Your Scent</h3>
                <div className="space-y-6">
                    <div>
                        <h4 className="text-lg font-semibold mb-3">Top Note (up to 3)</h4>
                        <div className="flex flex-wrap gap-2">
                            {SCENT_OPTIONS.top.map(note => (
                                <Button className="text-base p-3 h-auto" key={note} variant={selectedScents.top.includes(note) ? 'default' : 'outline'} onClick={() => toggleScent('top', note)}>{note}</Button>
                            ))}
                        </div>
                    </div>
                    <div>
                        <h4 className="text-lg font-semibold mb-3">Heart Note (up to 3)</h4>
                        <div className="flex flex-wrap gap-2">
                            {SCENT_OPTIONS.heart.map(note => (
                                <Button className="text-base p-3 h-auto" key={note} variant={selectedScents.heart.includes(note) ? 'default' : 'outline'} onClick={() => toggleScent('heart', note)}>{note}</Button>
                            ))}
                        </div>
                    </div>
                    <div>
                        <h4 className="text-lg font-semibold mb-3">Base Note (up to 3)</h4>
                        <div className="flex flex-wrap gap-2">
                            {SCENT_OPTIONS.base.map(note => (
                                <Button className="text-base p-3 h-auto" key={note} variant={selectedScents.base.includes(note) ? 'default' : 'outline'} onClick={() => toggleScent('base', note)}>{note}</Button>
                            ))}
                        </div>
                    </div>
                </div>
            ...
            <OptionGroup title="4. Select Packaging">
              {PACKAGING_OPTIONS.map(pack => (
                <OptionButton key={pack.id} selected={selectedPackaging.id === pack.id} onClick={() => setSelectedPackaging(pack)}>
                  <img src={pack.image} alt={pack.name} className="h-24 w-auto" />
                  <span>{pack.name}</span>
                </OptionButton>
              ))}
            </OptionGroup>
            ...
            <div className="mb-8">
              <h3 className="text-2xl font-heading font-semibold mb-4">5. Add a Custom Message</h3>
              <Textarea 
